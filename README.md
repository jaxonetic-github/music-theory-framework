...
The v9.1 repository suite contains **483 passing tests**: 399 plain-Node tests and 84 React DOM tests. `npm ci` installs 154 packages and audits 155; the current advisory database reports two moderate `postcss` findings through the Next.js fixture, for which npm offers only a breaking Next 16 upgrade. The production Vite build transforms **373 modules** and emits 672.49 kB JavaScript (182.61 kB gzip), a 29.62 kB / 6.87 kB gzip increase over v9.0. The Next.js 15.5.22 fixture compiles and statically prerenders successfully; its app route is 125 kB with 227 kB first-load JavaScript.
