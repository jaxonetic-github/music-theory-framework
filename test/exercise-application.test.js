...
test("module resolves active services per lifecycle and preserves explicit caller-owned services", async () => {
    const base = await runtime(); const kernel = new Kernel(); const ids = ["exercise.engine", "exercise.notation.engine", "rendering.engine"]; for (const id of ids) kernel.services.register(id, base.kernel.services.resolve(id)); const module = new ExerciseApplicationModule(); module.configure(kernel.context); const first = module.engine; assert.strictEqual(module.configure(kernel.context), module); module.dispose();
    const replacement = { generate: base.kernel.services.resolve("exercise.engine").generate.bind(base.kernel.services.resolve("exercise.engine")) }; kernel.services.register("exercise.engine", replacement, { replace: true }); module.configure(kernel.context); assert.notStrictEqual(module.engine, first); assert.strictEqual(module.engine.exerciseEngine, replacement); module.dispose();
});
...
test("module disposal preserves replacements, unrelated registrations, and reusable lifecycle", async () => {
    ...
    kernel.services.unregister("exercise.application.engine"); const injected = new ExerciseApplicationEngine({ exerciseEngine: source.kernel.services.resolve("exercise.engine"), notationEngine: source.kernel.services.resolve("exercise.notation.engine"), renderingEngine: source.kernel.services.resolve("rendering.engine") }); const owned = new ExerciseApplicationModule({ engine: injected }); owned.configure(kernel.context); owned.dispose(); assert.strictEqual(owned.engine, injected);
});

test("public namespace, descriptors, legacy Application, consumers, and Core remain independent", async () => {
    ...
    for (const forbidden of ["window", "document", "AudioContext", "navigator", "MIDIInput"]) assert.equal(forbidden in globalThis, false);
});
