...
test("web bootstrap installs and resolves the complete application workflow", async () => {
    ...
    assert.strictEqual(runtime.exerciseSetApplication, kernel.services.resolve("exercise.set.application"));
    assert.strictEqual(runtime.publishingEngine, kernel.services.resolve("publishing.engine"));
...
test("core public entry remains free of React and DOM imports", async () => {
    ...
    assert.equal(String(reactWebPackageDescriptor.version), "9.0.0");
