...
Curriculum Browser consumes the active template/curriculum services plus presentation-safe choices adapted from active Theory and Exercise catalogs. It filters and configures intent, expands a template, lesson, or complete curriculum, and sends the resulting ordinary `ExerciseSetRequest` through the existing worksheet application and presentation. Fixed constraints are non-editable, draft edits mark output stale, and failed or stale operations cannot destroy or replace a newer successful result. See [`curriculum/README.md`](curriculum/README.md).

`ResponsiveNotation` synchronously presents the already trusted authoritative SVG with title and description IDs derived from React's SSR-compatible `useId`, before effects or width observations run. The same trusted presentation utility rewrites only the renderer's known title, description, and root ARIA references for later responsive results, without mutating `row.content`. Hydration therefore retains the server IDs, missing or zero-width observers retain safe initial IDs, and stale completions cannot restore shared IDs. Multiple roots may pass `MusicTheoryApp.accessibilityIdPrefix`; the value is propagated and deterministically scoped to every practice and worksheet row, which guarantees isolation when React root-level `useId` prefixes are not coordinated.
