...
export function ResponsiveNotation({ row, renderingEngine, profile = "screen-regular", ResizeObserver }) {
    const accessibleId = useId(), observed = useNotationContainerWidth({ ResizeObserver }), operation = useRef(0), mounted = useRef(false), [state, setState] = useState(() => ({ content: row.content, error: null, width: null }));
