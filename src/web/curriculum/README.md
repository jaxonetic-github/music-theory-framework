...
Draft selection and overrides are distinct from the last authoritative expansion and worksheet result. Curriculum lesson controls select a unit first and derive lessons only from that unit, so repeated lesson IDs remain unambiguous. Changing units replaces the lesson scope with that unit's deterministic first lesson.
...
Filtering and active-catalog replacement normalize selection through one deterministic workflow helper. If the current entry remains available, its valid scope and overrides are retained without a material revision. Otherwise the first entry in deterministic catalog order is selected: curricula use its first unit and first lesson, while templates receive only the new template's configurable defaults. A no-match state clears the complete selection, disables generation, and is announced accessibly. Each effective intent change advances the material revision and invalidates pending work exactly once; presentation-only filter changes do neither.
