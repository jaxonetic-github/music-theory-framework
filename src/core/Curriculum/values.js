...
import { EXERCISE_SET_LIMITS } from "../ExerciseSet/index.js";

export const CURRICULUM_LIMITS = Object.freeze({ templates: 128, units: 24, lessonsPerUnit: 24, referencesPerLesson: EXERCISE_SET_LIMITS.itemsPerSection, totalLessons: EXERCISE_SET_LIMITS.sections, totalReferences: EXERCISE_SET_LIMITS.totalItems, parameters: 32, text: 1000 });
...
export class ExerciseTemplateExpansion {
    constructor({ template, request, parameters, exerciseSetRequest }={}) { if(!(template instanceof ExerciseTemplate)||!(request instanceof ExerciseTemplateRequest)||!exerciseSetRequest)throw new ValidationError("Invalid exercise template expansion."); Object.defineProperties(this,{id:{value:exerciseSetRequest.id,enumerable:true},template:{value:template,enumerable:true},request:{value:request,enumerable:true},parameters:{value:immutable(parameters),enumerable:true},exerciseSetRequest:{value:exerciseSetRequest,enumerable:true}}); Object.freeze(this); }
}

export class ExerciseTemplateReference {
    constructor({templateId,pluginId=null,overrides={},label=null}={}) { Object.defineProperties(this,{templateId:{value:id(templateId,"template reference"),enumerable:true},pluginId:{value:pluginId===null?null:id(pluginId,"template plugin reference"),enumerable:true},overrides:{value:immutable(overrides),enumerable:true},label:{value:label===null?null:text(label,"template reference label"),enumerable:true}});Object.freeze(this); }
}

export class CurriculumLesson {
    constructor({ id:value,title,description="",objective,difficulty="beginner",tags=[],prerequisites=[],templates=[] }={}) { const lessonId=id(value,"lesson id");if(!Array.isArray(templates)||!templates.length||templates.length>CURRICULUM_LIMITS.referencesPerLesson)throw new ValidationError(`Lesson "${lessonId}" requires 1 through ${CURRICULUM_LIMITS.referencesPerLesson} template references.`);const refs=templates.map((entry,index)=>{if(!entry||typeof entry!=="object"||Array.isArray(entry))throw new ValidationError(`Lesson "${lessonId}" template ${index+1} must be an object.`);return entry instanceof ExerciseTemplateReference?entry:new ExerciseTemplateReference(entry);});Object.defineProperties(this,{id:{value:lessonId,enumerable:true},title:{value:text(title,"Lesson title"),enumerable:true},description:{value:text(description,"Lesson description",false),enumerable:true},objective:{value:text(objective,"Lesson objective"),enumerable:true},difficulty:{value:DifficultyLevel.from(difficulty),enumerable:true},tags:{value:list(tags,"Lesson tags"),enumerable:true},prerequisites:{value:list(prerequisites,"Lesson prerequisites"),enumerable:true},templates:{value:Object.freeze(refs),enumerable:true}});Object.freeze(this); }
}
...
export class Curriculum {
    constructor({ id:value,title,description,version="1.0.0",objective,difficulty="beginner",tags=[],prerequisites=[],units=[],metadata={} }={}) { const curriculumId=id(value,"curriculum id");if(!Array.isArray(units)||!units.length||units.length>CURRICULUM_LIMITS.units)throw new ValidationError(`Curriculum "${curriculumId}" requires a bounded non-empty unit list.`);const normalized=units.map(value=>value instanceof CurriculumUnit?value:new CurriculumUnit(value));if(new Set(normalized.map(value=>value.id)).size!==normalized.length)throw new ValidationError(`Curriculum "${curriculumId}" contains duplicate unit IDs.`);const lessonCount=normalized.reduce((total,unit)=>total+unit.lessons.length,0),referenceCount=normalized.reduce((total,unit)=>total+unit.lessons.reduce((sum,lesson)=>sum+lesson.templates.length,0),0);if(lessonCount>CURRICULUM_LIMITS.totalLessons)throw new ValidationError(`Curriculum "${curriculumId}" has ${lessonCount} lessons; expandable curricula support at most ${CURRICULUM_LIMITS.totalLessons}.`);if(referenceCount>CURRICULUM_LIMITS.totalReferences)throw new ValidationError(`Curriculum "${curriculumId}" has ${referenceCount} template references; expandable curricula support at most ${CURRICULUM_LIMITS.totalReferences} total items.`);assertAcyclic(normalized,"Unit");for(const unit of normalized)assertAcyclic(unit.lessons,"Lesson");Object.defineProperties(this,{id:{value:curriculumId,enumerable:true},title:{value:text(title,"Curriculum title"),enumerable:true},description:{value:text(description,"Curriculum description"),enumerable:true},version:{value:text(version,"Curriculum version"),enumerable:true},objective:{value:text(objective,"Curriculum objective"),enumerable:true},difficulty:{value:DifficultyLevel.from(difficulty),enumerable:true},tags:{value:list(tags,"Curriculum tags"),enumerable:true},prerequisites:{value:list(prerequisites,"Curriculum prerequisites"),enumerable:true},units:{value:Object.freeze(normalized),enumerable:true},metadata:{value:new CurriculumMetadata(metadata),enumerable:true}});Object.freeze(this); }
}

export class CurriculumExpansionRequest {
    constructor({ curriculumId, pluginId="core.curriculum.builtins", unitId=null, lessonId=null, overrides={} }={}) { const normalizedUnit=unitId===null?null:id(unitId,"unit request id"),normalizedLesson=lessonId===null?null:id(lessonId,"lesson request id");if(normalizedLesson&&!normalizedUnit)throw new ValidationError(`Curriculum lesson "${normalizedLesson}" requires its unitId because lesson identity is scoped to a unit.`);Object.defineProperties(this,{curriculumId:{value:id(curriculumId,"curriculum request id"),enumerable:true},pluginId:{value:id(pluginId,"curriculum plugin id"),enumerable:true},unitId:{value:normalizedUnit,enumerable:true},lessonId:{value:normalizedLesson,enumerable:true},overrides:{value:immutable(overrides),enumerable:true}});Object.freeze(this); }
}
export class CurriculumExpansionResult {
    constructor({ request,curriculum,exerciseSetRequest,metadata={} }={}) { if(!(request instanceof CurriculumExpansionRequest)||!(curriculum instanceof Curriculum)||!exerciseSetRequest)throw new ValidationError("Invalid curriculum expansion result.");Object.defineProperties(this,{id:{value:exerciseSetRequest.id,enumerable:true},request:{value:request,enumerable:true},curriculum:{value:curriculum,enumerable:true},exerciseSetRequest:{value:exerciseSetRequest,enumerable:true},metadata:{value:new CurriculumMetadata(metadata),enumerable:true}});Object.freeze(this); }
}
