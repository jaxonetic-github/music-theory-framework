...
The renderer owns a deterministic set of SVG paths and geometric primitives for clefs, accidentals, noteheads, stems, flags, augmentation dots, rests, ledger lines, staff lines, and barlines. Written letter name and octave determine vertical placement relative to the active clef; MIDI is not used for staff position. Key-signature state and per-measure accidental state control visible accidentals. Chord heads share one rhythmic anchor. Within each maximal chain of consecutive written staff positions, heads alternate base/displaced sides beginning again at the base side for each independent chain. Up-stem chords displace alternating heads left; down-stem chords mirror them right. The shared stem attaches to the outer base-side head, while accidental columns, ledger lines, and augmentation dots use each member's final center. Chords use one stem. Isolated short values receive individual flags; beaming is intentionally deferred.
