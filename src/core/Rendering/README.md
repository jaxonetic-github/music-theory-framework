...
The renderer maps alterations -2 through +2 explicitly to double-flat, flat, natural, sharp, and double-sharp vector glyphs and rejects unsupported values. Within each staff and measure, accidental state is shared across voices and keyed by written diatonic step plus octave. Rendering traverses the LayoutPlan's canonical `eventPlacements` sequence directly, so `next`-edge precedence cannot be overridden by an independent offset sort. Every chord consults the state that existed before the chord for all members, then applies deterministic position-sorted updates atomically. Key signatures supply per-step defaults across octaves, while measure overrides reset at each barline. Changed key and meter headers are emitted at their allocated measure boundary and repeated with the active clef at a new visual system.
